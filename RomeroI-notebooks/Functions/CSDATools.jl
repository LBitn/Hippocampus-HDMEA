...
"""
    SearchDir( path::String, key::String ) -> Vector{String}
        Find inside the given path, the files with the key word on their name and returns
        a vector of strings with the full name of those files (complete path)
"""
SearchDir( path::String, key::String ) = filter(
    x -> endswith( x, key ), readdir( path; join = true ) );
# ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ #
function Binarizar( WmM )
    nModes = 9;
    fModes = [
        UnimodalRosin( ), MinimumIntermodes( ), Intermodes( ), MinimumError( ), Moments( ),
        Otsu( ), Entropy( ), Balanced( ), Yen( ) ];
    TPlots = [ ];
    TChannels = [ ];
    @suppress begin
    for i in 1:nModes
        f = fModes[ i ];
        t = find_threshold( WmM, f; nbins = length( WmM ) ); t = round( t, digits = 3 )
        T = zeros( Int, size( WmM, 1 ) );
        T[ WmM .>= t ] .= 1;
        number = length( findall( WmM .>= t ) );
        Mode = string( f )[ 1 : ( end - 2 ) ];
        P = Zplot( T, "W" );
        P = plot!( title = "$Mode, \n$number channels, t = $t",
            cbar = :none, titlefontsize = 6 )
        push!( TPlots, P ); push!( TChannels, findall( T .== 1 ) );
    end
    end
    return TPlots, TChannels
end
# ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ #
function BinarizarIteracion( WmM, limits, confinaza, max = 10 );
    # plot( TPlots..., wsize = ( 700, 700 ) ); savefig( "Vuelta$iThresholding.png" )
    nChs = length( WmM );
    x = copy( WmM );
    results = [ ];
    i = 1
    _, TChannels = Binarizar( x );
    coef = 1 .- ( length.( TChannels ) .- limits[ 1 ] ) ./ ( limits[ 2 ] - limits[ 1 ] );
    push!( results, coef )
    aux = vcat( TChannels[ confinaza[ 2 ] .> coef .> confinaza[ 1 ] ]... );
    V = collect( values( countmap( aux ) ) ); K = collect( keys( countmap( aux ) ) );
    i = i + 1;
    x = zeros( Int, nChs ); x[ K ] .= V;
    _, TChannels = Binarizar( x );
    coef = 1 .- ( length.( TChannels ) .- limits[ 1 ] ) ./ ( limits[ 2 ] - limits[ 1 ] );
    while ( results[ i - 1 ] != coef ) & ( i < max )
        push!( results, coef )
        aux = vcat( TChannels[ confinaza[ 2 ] .> coef .> confinaza[ 1 ] ]... );
        V = collect( values( countmap( aux ) ) ); K = collect( keys( countmap( aux ) ) );
        i = i + 1;
    ...
    R = results[ end ];
    FinalChs = TChannels[ length.( TChannels ) .!= length( WmM ) ];
    return R, FinalChs, i
end
# ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ ~~ ~ #
